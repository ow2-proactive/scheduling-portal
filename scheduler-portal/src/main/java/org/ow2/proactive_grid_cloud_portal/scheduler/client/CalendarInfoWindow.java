...
public class CalendarInfoWindow {
    ...
    private void build() {
        ...
        text.setContents(createContext());
        text.setWidth100();
        ...
        this.window.setTitle("Calendar integration");
    ...
    private final static String CALENDAR_CONTEXT =
            "<h1>ProActive Scheduling & Orchestration: integration with Calendars </h1>" +
            "<font size=\"3\">"+
            "Import this URL in your favorite Calendar (Outlook, Google, Thunderbird, etc.): <br><br>" +
            "http://" + "@hostURL@" + ":5232/" + "@login@"  + "/calendar.ics/"+
            "<br><br>"+
            "<a target='_blank' href='http://doc.activeeon.com/" + "@docVersion@"  +"/user/ProActiveUserGuide.html#_calendar_service'>" +
                    "See calendar Documentation and Installation</a> "
            +"</font>";

    private String getDocumentVersion(){
        return Config.get().getVersion().contains("SNAPSHOT") ? "latest"
                : Config.get().getVersion();
    }

    private String fillTemplate(){
        return CALENDAR_CONTEXT.replace("@hostURL@",  com.google.gwt.user.client.Window.Location.getHostName())
                .replace("@login@", LoginModel.getInstance().getLogin())
                .replace("@docVersion@", getDocumentVersion());
    }
}
