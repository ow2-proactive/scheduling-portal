...
public class FlatSubmitWindow {
    ...
    private void build() {
        ...
        final Label label = new Label("Submit file containing commands, each line will represent a single native task:");
        label.setHeight(30);
        label.setWidth100();
                ...
                form.setValue("flatCallback", JSUtil.register(new JSUtil.JSCallback() {
                    ...
                    public void execute(JavaScriptObject obj) {
                        ...
                        if (js.containsKey("id") && js.get("id").isNumber() != null) {
                            ...
                            LogModel.getInstance().logMessage("Successfully submitted flat job " + name + ": " + id);
                            controller.getExecutionController().getJobsController().addSubmittingJob(id, name);
                        } else {
